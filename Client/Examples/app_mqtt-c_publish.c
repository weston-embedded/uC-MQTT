...
#define  APP_MQTTc_MSG_QTY                         1u

#define  APP_MQTTc_MSG_LEN_MAX                   128u
#define  APP_MQTTc_PUBLISH_RX_MSG_LEN_MAX        512u

                                                                /* Domain to which to publish.                          */
#define  APP_MQTTc_DOMAIN_PUBLISH                   "domain/publish_topic"

#define  APP_MQTTc_PUBLISH_TEST_MSG                 "test publish"
#define  APP_MQTTc_PUBLISH_TEST_QoS                2u
...
static  CPU_INT08U   AppMQTTc_MsgPublishRxBuf[APP_MQTTc_PUBLISH_RX_MSG_LEN_MAX];
                                                  ...
                                                  MQTTc_ERR    err)
{
    ...
    if (err != MQTTc_ERR_NONE) {
        ...
        MQTTc_Publish(p_conn,
                      ...
                      Str_Len(APP_MQTTc_PUBLISH_TEST_MSG),
                     &err_mqttc);
        if (err_mqttc != MQTTc_ERR_NONE) {
                                                  ...
                                                  MQTTc_ERR    err)
{
    ...
    if (err != MQTTc_ERR_NONE) {
        printf("PublishCmpl callback called with error (%i). CANNOT re-send message.\n\r", err);
    } else {
        printf("PublishCmpl callback called. Re-sending same message.\n\r");
